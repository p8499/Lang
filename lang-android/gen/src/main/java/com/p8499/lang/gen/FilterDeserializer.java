...
public class FilterDeserializer extends JsonDeserializer<FilterExpr> {
  ...
  protected FilterExpr recursiveDeserialize(JsonNode node) {
    ...
    if ("and".equals(opNode.asText())
        || "or".equals(opNode.asText())
        || "not".equals(opNode.asText())) {
      ...
      return expr;
    } else if ("equal".equals(opNode.asText())
        || "greater".equals(opNode.asText())
        || "less".equals(opNode.asText())
        || "greaterEqual".equals(opNode.asText())
        || "lessEqual".equals(opNode.asText())
        || "isEmpty".equals(opNode.asText())
        || "contain".equals(opNode.asText())
        || "startWith".equals(opNode.asText())
        || "endWith".equals(opNode.asText())
        || "exists".equals(opNode.asText())) {
