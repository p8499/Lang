...
import com.p8499.lang.server.mapper.local.WordMapper;

public abstract class WordControllerBase {
  protected static final String path = "api/word/";
  protected static final String attachPath = "api/word_attachment/";
  protected static final String pathKey = "{woid}";
  ...
  public String get(
    ...
    WordMask maskObj =
        mask == null || mask.equals("")
            ? new WordMask().all(true)
            : jackson.readValue(mask, WordMask.class);
    Word bean = onGet(session, request, response, woid, maskObj);
    return jackson.writeValueAsString(bean);
  }

  protected abstract Word onGet(
      HttpSession session,
      HttpServletRequest request,
      HttpServletResponse response,
      Integer woid,
  ...
  protected Word baseGet(Integer woid, WordMask mask) {
    return wordMapper.get(woid, mask);
  }
  ...
  public String add(
      ...
      HttpServletRequest request,
      HttpServletResponse response,
      @RequestBody Word bean)
      throws Exception {
    onAdd(session, request, response, bean);
    return jackson.writeValueAsString(bean);
  }

  protected abstract Word onAdd(
      HttpSession session, HttpServletRequest request, HttpServletResponse response, Word bean)
      throws Exception;

  protected Word baseAdd(Word bean) {
  ...
  public String update(
    ...
    WordMask maskObj =
        mask == null || mask.equals("")
            ? new WordMask().all(true)
            : jackson.readValue(mask, WordMask.class);
    onUpdate(session, request, response, woid, bean, maskObj);
    return jackson.writeValueAsString(bean);
  }

  protected abstract Word onUpdate(
      HttpSession session,
      HttpServletRequest request,
      HttpServletResponse response,
      Integer woid,
  ...
  protected Word baseUpdate(Word bean, WordMask mask) {
    Set<ConstraintViolation<Word>> violationSet =
        validatorFactory.getValidator().validate(bean, Update.class);
    for (ConstraintViolation<Word> violation : violationSet) {
      if (mask.get(violation.getPropertyPath().toString())) {
        return null;
      }
    }
    wordMapper.update(bean, mask);
    return bean;
  }
  ...
  public void delete(
      ...
      @PathVariable Integer woid)
      throws Exception {
    onDelete(session, request, response, woid);
  }

  protected abstract void onDelete(
      HttpSession session, HttpServletRequest request, HttpServletResponse response, Integer woid)
      throws Exception;

  protected boolean baseDelete(Integer woid) {
    return wordMapper.delete(woid);
  }
  ...
  protected abstract List<Word> onQuery(
      HttpSession session,
      HttpServletRequest request,
      HttpServletResponse response,
      FilterExpr filter,
  ...
  protected long baseCount(FilterExpr filter) {
    return wordMapper.count(filter == null ? null : filter.toString());
  }

  protected List<Word> baseQuery(
      FilterExpr filter, OrderByListExpr orderByList, long start, long count, WordMask mask) {
    return wordMapper.query(
  ...
  public void downloadAttach(
      ...
      throws Exception {
    InputStream input = inputStream(session, request, response, woid, name);
    if (input == null) {
      return;
    }
  ...
  public void uploadAttach(
      ...
      @RequestParam(required = true) String name)
      throws Exception {
    OutputStream output = outputStream(session, request, response, woid, name);
    if (output == null) {
      return;
    }
  ...
  public void deleteAttach(
      ...
      @RequestParam(required = true) String name)
      throws Exception {
    onDeleteAttach(session, request, response, woid, name);
  }
  ...
  public String listAttaches(
      ...
      @PathVariable Integer woid)
      throws Exception {
    List<String> result = onListAttaches(session, request, response, woid);
